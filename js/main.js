...
function getSpeedTab(geoData) {
	...
	for (let i=0; i<geoData.paths[geoData.focus].features[0].geometry.coordinates.length-1; i++){
		speedTab.push(DistanceBetween2Points(shortcut.geometry.coordinates[i],shortcut.geometry.coordinates[i+1])/differenceBetween2coordTimes(shortcut.properties.coordTimes[i+1],shortcut.properties.coordTimes[i]));
	}
	console.log(speedTab);
}

function speedTabToIntervals(tab){
	let minTab = tab.min();
	let maxTab = tab.max();
	let range = maxTab-minTab;
	let tabInterval = [];
	for(let i=0; i<tab.length; i++){
			if(tab[i]<(minTab+range/9)){
				tabInterval.push(0);
			} else if(tab[i]<(minTab+range*2/9) && tab[i]>=(minTab+range/9)) {
				tabInterval.push(1);
			} else if(tab[i]<(minTab+range*3/9) && tab[i]>=(minTab+range*2/9)) {
				tabInterval.push(2);
			} else if(tab[i]<(minTab+range*4/9) && tab[i]>=(minTab+range*3/9)) {
				tabInterval.push(3);
			}  else if(tab[i]<(minTab+range*5/9) && tab[i]>=(minTab+range*4/9)) {
				tabInterval.push(4);
			}  else if(tab[i]<(minTab+range*6/9) && tab[i]>=(minTab+range*5/9)) {
				tabInterval.push(5);
			} else if(tab[i]<(minTab+range*7/9) && tab[i]>=(minTab+range*6/9)) {
				tabInterval.push(6);
			} else if(tab[i]<(minTab+range*8/9) && tab[i]>=(minTab+range*7/9)) {
				tabInterval.push(7);
			} else {
				tabInterval.push(8);
			}
