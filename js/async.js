...
function loadingTimeActivate(geoData, activate) {
    setInterval(loadingTime, 20, activate);
    return geoData;
}

function loadingTime(activate) {
	if (activate) {
		let effective = document.querySelector("#loadImage > div > div.loadEffect");
		let parent = effective.parentElement;
		...
		while (effective != null) {
			effective = effective.previousElementSibling;
			effectiveIndex++;
		}
		while (parent != null) {
		...
		switch (parentIndex) {
			...
			default: {
				console.error(`Index ${parentIndex} not found`);
			}
		}
		
		if (newColIndex !== undefined) {
			document.querySelector("#loadImage > div > div.loadEffect").classList.remove("loadEffect");
			document.getElementById("loadImage").children[newColIndex].children[newRowIndex].classList.add("loadEffect");
		}
	}
}
